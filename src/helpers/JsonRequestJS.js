...
const JsonRequest = ({
  ...
  return ({
    post: async (to, parametersForThisRequest) => responseWrapper(`${mainPath}/${to}`, {
      ...options, body: JSON.stringify(parametersForThisRequest.body), method: 'post',
    }),
    get: async (from, parametersForThisRequest) => responseWrapper(
      setUrl(`${mainPath}/${from}?`, combineParams(parametersForThisRequest, params)),
      { ...options, method: 'get' },
      handleResponse,
    ),
    getByID: async (from, parametersForThisRequest) => responseWrapper(
      setUrl(`${mainPath}/${from}?`, combineParams(parametersForThisRequest, params)), { ...options, method: 'get' }, handleResponse,
    ),
    deleteByParameters: async (from, parametersForThisRequest) => responseWrapper(
      setUrl(`${mainPath}/${from}?`, combineParams(parametersForThisRequest, params)),
